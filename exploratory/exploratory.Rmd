...
ggplot(sub_df,aes(x=factor(level,levels=c("ASV", "OTU","Genus","Family","Order","Class", "Phylum")),
  ...
  coord_flip() +
  geom_hline(yintercept = 0.5,color="grey",lty="dashed") 
ggsave("./figures/rf_with_stats_sideways.png",width = 5,height=4,units="in")
```
...
```{r prevalence_preprocess,echo=F,message=F}
get_prevalence_preprocess <- function(file,file_proc,level){
  ...
  n_samp <- nrow(data_proc)
  prev <- select(data,keep_names) %>% 
...
# prev_df <- get_prevalence_preprocess("./data/final/phylum/input_data.csv",
#                                      "./data/final/phylum/input_data_preproc.csv",
#                                      "phylum") %>%
#   bind_rows(get_prevalence_preprocess("./data/final/class/input_data.csv",
#                                       "./data/final/class/input_data_preproc.csv",
#                                       "class")) %>%
#   bind_rows(get_prevalence_preprocess("./data/final/order/input_data.csv",
#                                       "./data/final/order/input_data_preproc.csv",
#                                       "order")) %>%
#   bind_rows(get_prevalence_preprocess("./data/final/family/input_data.csv",
#                                       "./data/final/family/input_data_preproc.csv",
#                                       "family")) %>%
#   bind_rows(get_prevalence_preprocess("./data/final/genus/input_data.csv",
#                                       "./data/final/genus/input_data_preproc.csv",
#                                       "genus")) %>%
#   bind_rows(get_prevalence_preprocess("./data/final/otu/input_data.csv",
#                                       "./data/final/otu/input_data_preproc.csv",
#                                       "otu")) %>%
#   bind_rows(get_prevalence_preprocess("./data/final/asv/input_data.csv",
#                                       "./data/final/asv/input_data_preproc.csv",
#                                       "asv"))
# write_csv(prev_df,"./data/analysis/exploratory3/prevalence_by_level_preprocess.csv")

prevalence_prepro <- read_csv("./data/analysis/exploratory3/prevalence_by_level_preprocess.csv")

prevalence_prepro$level <- factor(prevalence_prepro$level,levels=levels)

ggplot(prevalence_prepro,aes(x=level,y=prevalence,fill=level)) + 
  #geom_jitter() +
  geom_boxplot() +
  stat_summary(fun = mean, color="red",geom = "point", shape = 18, size = 4,show.legend = FALSE) +
  theme_bw() + xlab("") +
  theme(legend.position = "none",
        axis.text = element_text(size=14),
        axis.title.y = element_text(size=14))
ggsave("./data/analysis/exploratory3/prevalence_preprocess.png",width=5,height=4,units = "in")

ggplot(prevalence_prepro,aes(x=prevalence,fill=level)) +
  ...
  theme(legend.position = "none") 
ggsave("./data/analysis/exploratory3/prevalence_histogram.png",width=5,height=4,units = "in")

ggplot(prevalence_prepro,)

tax_count <- prevalence_prepro %>% 
  group_by(level) %>% 
...
#   group_by(level) %>% 
#   count(prevalence) %>% 
#   inner_join(tax_count,by="level") %>% 
#   mutate(pct_OTU = n/numOTU) %>% 
#   mutate(prevalence = round(prevalence,digits=2)) %>% 
#   ggplot(aes(x=prevalence,y=pct_OTU)) +
#     geom_line() +
#     facet_wrap(~level,scales = "free")
  #ggplot(aes(x=prevalence,y=n)) +
...
level_nOTU <- NULL
for(l in levels){
  sub_data <- prevalence_prepro %>% 
    filter(level==l)
  hist <- hist(sub_data$prevalence,breaks = seq(0,1,0.25))
...
ggsave("./data/analysis/exploratory3/bin_prevalence.png")
