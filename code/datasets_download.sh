...
# Need to get the column that contains the run file names that start wtih SRR (SRA) or ERR (ENA).
# The column position is not consistent across datasets, so need to remove column header and use
# sed to isolate column
SRRS=`tail -n +2 $DATA/sra_info.tsv | sed -E "s/.*\t(SRR\S*)\t.*/\1/"`
