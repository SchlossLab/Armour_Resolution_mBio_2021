...
if( args$taxonomy %in% c("otu","asv") ){
	data <- data[,colSums(data != 0) > 1]
}

data_proc <- mikropml::preprocess_data(data, 'dx')

#write_csv(as.data.frame(data_proc$removed_feats),paste0("./data/",args$taxonomy,"/input_data_removed.csv"),col_names=FALSE)
write_csv(data_proc$dat_transformed,paste0("./data/",args$taxonomy,"/input_data_preproc.csv"))
