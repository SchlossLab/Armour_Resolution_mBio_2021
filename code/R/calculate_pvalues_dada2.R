...
dada2_files <- list.files(path='./data/dada2/process',pattern='combined-dada2-.*',full.names = TRUE)
mASV_files <- list.files(path= './data/process', pattern='combined-asv-.*', full.names = TRUE)


all.df <- map_df(c(dada2_files,mASV_files), read_files)

#perm_p_value_model(all.df, "rf", "genus", "otu")
#perm_p_value_level(all.df, "rpart2","rf","genus")

p.table.level <- NULL
p.table.level <- expand_grid(x=2,
                             y=1:length(models),
                             z=1:length(models)) %>%
  filter(y < z) %>%
  ...
  summarize(p_value = perm_p_value_level(all.df,model1,model2,level),
            .groups="drop")
write.csv(p.table.level,paste0(outdir,"dada2_pvalues_by_level.csv"),row.names = F,quote = F)
...
p.table.model <- expand_grid(x=1:length(levels),
  ...
  summarize(p_value = perm_p_value_model(all.df,model,level1,level2),
            .groups="drop")
write.csv(p.table.model,paste0(outdir,"dada2_pvalues_by_model.csv"),row.names = F,quote = F)
