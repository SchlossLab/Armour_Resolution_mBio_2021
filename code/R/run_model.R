...
run_model <-
    function(seed,
        ...
        dir.create(file.path("data", "temp", taxonomy), showWarnings = FALSE)
        set.seed(seed)
...
#        tic("model")
	    ...
	    #write_csv(path = paste0("data/", taxonomy,"/",model,".", seed, ".csv"))
	     write_csv(path = paste0("data/temp/", taxonomy,"/",model,".", seed, ".csv"))
        # ------------------------------------------------------------------
